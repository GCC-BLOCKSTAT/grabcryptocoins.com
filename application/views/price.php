...
<section class="ls section section_padding_bottom_75">
    <div class="container">
        <div class="row">
            <div class="col-sm-12" id="hideCurrencyDetails">
                <div class="table-responsive">
                    <a href="javascript:void(0)" class="refresh" id="currencyRefresh"><i class="fa fa-refresh"></i> refresh</a>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php if(isset($_SESSION['user']) && !empty($_SESSION['user'])){ ?>
                                    ...
                                    <a class='theme_button small-btn inverse colorFrame' href="<?php echo base_url('dashboard/addBuy/'.$currency->symbol.'/'.$buydSell.'/1'); ?>">Buy</a>
                                <?php }else{ ?>
                               ...
                               <?php } ?>
                                ...
                                <td><?php if(isset($_SESSION['user']) && !empty($_SESSION['user'])){ ?>
                                    ...
                                    <a class='theme_button small-btn color2 colorFrame' href="<?php echo base_url('dashboard/addSell/'.$currency->symbol.'/'.$selldSell.'/2'); ?>">Sell</a>
                                    <?php }else{ ?>
                               ...
                               <?php }else{ ?>
                                   <input type='hidden' value="" id="getUserName">                                 
                              <?php } ?>
                            </tr>
                            <?php } ?>
                        </tbody>
            ...
            <div class="text-center"><img src="<?php echo base_url('assets/img/loader.gif'); ?>" style="display: none; margin: 0 auto;" id="imgLoaderRefresh"></div>  
            <div id="showCurrencyDetails"></div>
        </div>
    </div>
</section>

<!--<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true"  id="myModalNotification">
    <div class="modal-dialog">
       Modal content
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"><i class="fa fa-exclamation-circle"></i>Notification</h4>
        </div>
        <div class="modal-body">
         <?php // if(!empty($notification->content)){
//          echo $notification->content; }else{ echo 'No Notification'; } ?>
        </div>
...
//        var username_value = $('#getUserName').val();
//        if(username_value != ''){
...
//    });
    ...
    $(document).ready(function(){    
        ...
        $('#currencyRefresh').click(function(){
            var curr_url = '<?php echo base_url('BuySell/getCurrencyRefresh'); ?>';
            $('#imgLoaderRefresh').css('display', 'block');
            $('#hideCurrencyDetails').hide();
           $.ajax({
                type: "POST",
                url: curr_url,
                data: {},
                success: function (response) {
                   $('#imgLoaderRefresh').css('display', 'none');
                   $('#hideCurrencyDetails').hide();
                   $('#showCurrencyDetails').html(response);
                }   
            });
        });
...
<script type="text/javascript">
   $(document).ready(function () {
    $.noConflict();
        $(".colorFrame").colorbox({iframe: true, width: "50%", height: "86%"});
    });
</script>