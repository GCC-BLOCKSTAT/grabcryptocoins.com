...
<!--                    <h3 class="box-title">Make Account</h3>-->
                ...
                <form class="popupAdd" id="popupBuyMoney">
                    <div class="form-group">
                        <label for="buy">Withdraw Method:</label>
                        <input type="text" name="wallet" id="popupbuyingamount" value="<?php echo $row->withmethod; ?>" class="form-control" readonly="readonly">
                        <span class="grey" style="color:red;" id="errAmount"></span>
                    </div>
                    <div class="form-group">
                        <label for="buy">Details: <span style="color:red;"> <?php if($row->withmethod == 'bank' || $row->withmethod == 'paypal'){
                            echo getWithdrawMethodDetails($row->withdrawlid, $user_id, $row->withmethod);
                        }else{ echo '-------'; } ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="amount" id="popupbuyingamount" value="<?php echo $row->amount; ?>" class="form-control" readonly="readonly">
                        <span class="grey" style="color:red;" id="errAmount"></span>
                    </div>
                    
                    <input type="hidden" name="buyID" id="buyID" value="<?php echo $row->withdrawlid; ?>">
                    <input type="hidden" name="stauseAorove" id="stauseAorove" value="<?php echo $row->withdrawlid; ?>">
                    <input type="hidden" name="statusID" id="stauseAorove" value="<?php echo $row->withdrawstatus; ?>">
                    ...
                    <div class="form-group" id="successShow" style="display:none;width:100%">
                        <label for="buy">Transaction ID:</label>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('form#popupBuyMoney').submit(function (event) {
            ...
            if(status == '1'){
                if(txn == '' || txn == ' '){
                    $("#errTxn").text('Please Enter Transaction ID');
                    return false;
                }else{
                    $("#errTxn").hide();
                }
            ...
            var burl = '<?php echo base_url('admin/Withdraw/statusView'); ?>';
                $.ajax({
                    type: "POST",
                    url: burl,
                    contentType: false,
                    ...
                    success: function (response) {
                        if (response == '2') {
                            $('#suceesLevelAcc').html('Withdraw Amount Updated Successfully.');
                            setTimeout(function() {
